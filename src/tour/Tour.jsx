...
export default function Tour({ src, edit = false }) {
  ...
  // drž ref synchronizovaný so stavom
          ...
          plugins: [
            ...
            [
              VirtualTourPlugin,
              {
                positionMode: 'manual',
                renderMode: '3d',
                nodes: data.nodes.map((n) => toPsvNode(n, baseUrl)),
                startNodeId: startId,
  ...
  const addNode = useCallback((name, file) => {
    ...
    setTour((prev) => {
      ...
      vtRef.current?.setNodes(
        [...prev.nodes, node].map((n) => toPsvNode(n, baseUrl)),
        prev.startNodeId || id,
      );
  ...
  const removeNode = useCallback((nodeId) => {
    ...
    setTour(next);
    vtRef.current?.setNodes(nodes.map((n) => toPsvNode(n, baseUrl)), stayOn);
